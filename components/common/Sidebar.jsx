...
export default function Sidebar({ links, title = 'PayRemind', userRole = 'User' }) {
  const pathname = usePathname();
  const { logout } = useAuth(); // Assuming logout is exposed

  return (
    <div className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full min-h-screen fixed left-0 top-0">
      {/* Brand */}
      {/* Brand */}
      <div className="h-16 flex items-center px-6 border-b border-gray-200 dark:border-gray-700">
        <Link href="/" className="flex items-center gap-2">
            <img src="/Payremind logo.svg" alt="PayRemind" className="h-10" />
        </Link>
      </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-3 w-full p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
            ...
            <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{userRole}</p>
                <button onClick={logout} className="text-xs text-red-500 hover:text-red-600 flex items-center gap-1 mt-0.5">
